...
void LinkerObject::link(map<string, h192> const& _libraryAddresses)
{
	...
	for (auto const& linkRef: linkReferences)
		if (h192 const* address = matchLibrary(linkRef.second, _libraryAddresses))
			copy(address->data(), address->data() + 20 + 4, bytecode.begin() + linkRef.first);
		else
...
string LinkerObject::toHex() const
{
	...
	for (auto const& ref: linkReferences)
	{
		...
		hex[pos] = hex[pos + 1] = hex[pos + 38 + 8] = hex[pos + 39 + 8] = '_';
...
LinkerObject::matchLibrary(
	...
	map<string, h192> const& _libraryAddresses
)
{
